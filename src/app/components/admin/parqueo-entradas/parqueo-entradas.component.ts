...
@Component({
  selector: 'app-parqueo-entradas',
  templateUrl: './parqueo-entradas.component.html',
  styleUrls: ['./parqueo-entradas.component.css']
})
export class ParqueoEntradasComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  ngOnInit(): void {
    if(localStorage.getItem("Sucursales") != undefined){
      ...
      setTimeout(()=>{
        this.listaEntradasVehiculosByIdParqueo();
      }, 100);
  ...
  listaEntradasVehiculosByIdParqueo()
  {
    ...
    this.http.post(this.url+"parqueo/listaEntradasVehiculosByIdParqueo", parametros).subscribe((datos_recibidos:any) => {
      Swal.close();
      ...
      if(datosMysql === undefined){
        if(datosNodejs === undefined){
          this.listaEntradasParqueo = datos;
  ...
  reporteEntradasVehiculoByParqueo() {
    Swal.fire({title: 'Generando reporte PDF',text: 'Aguarde unos segundos . . .',allowOutsideClick: false,willOpen: () => {Swal.showLoading()}});
    //id_sucursal, id_parqueo, estado, id_usuario, fecha_inicio, fecha_fin, id_usuario_impresion
    const params = new HttpParams({
      fromObject: {
      "id_sucursal": this.listaEntradasParqueo[0].id_sucursal,
      "id_parqueo": this.id_parqueo,
      "estado": 1,
      "id_usuario": this.id_usuario,
      "fecha_inicio": this.enviar_fecha_inicio,
    ...
    this.http.post(this.url+"parqueo/reporteEntradasVehiculoByParqueo", params, {responseType: 'arraybuffer',headers:headers}).subscribe((datos_recibidos:any) => {
