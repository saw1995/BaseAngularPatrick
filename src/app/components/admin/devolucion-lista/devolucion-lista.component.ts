...
@Component({
  selector: 'app-devolucion-lista',
  templateUrl: './devolucion-lista.component.html',
  styleUrls: ['./devolucion-lista.component.css']
})
export class DevolucionListaComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  constructor(private route:ActivatedRoute, private servicioSucursal: ServicioSucursalService, public router: Router, private http:HttpClient) { }
  ...
  ngOnInit(): void {
    if(localStorage.getItem("Sucursales") != undefined){
      ...
      setTimeout(()=>{
        this.listaDevolucionByIdSucursal();
      }, 100);
  ...
  click_seleccionDevolucion(id_recepcion:string){
    id_recepcion = id_recepcion + "";
    this.router.navigate(['/sucursal/recepcion/detalle/', encryptNumber(this.id_sucursal), encryptNumber(id_recepcion)]);
  }

  listaDevolucionByIdSucursal(){
    Swal.fire({title: 'Buscando devoluciones. . .',text: 'Aguarde unos segundos . . .',allowOutsideClick: false,willOpen: () => {Swal.showLoading()}});
    let parametros = {
    ...
    this.http.post(this.url+"devolucion/listaDevolucionByIdSucursal", parametros).subscribe((datos_recibidos:any) => {
